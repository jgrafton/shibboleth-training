...
if [[ "$response" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
  ...
  #kill, if running, and remove RDBMS container
  docker ps | grep idp_rdbms &>/dev/null
  if [  $? == '0' ]; then
    #get container ID
    export contid2=$(docker ps | grep idp_rdbms | cut -f 1 -d ' ')
    docker kill ${contid2} &>/dev/null
    docker rm ${contid2} &>/dev/null
  else
    #check if an old container is present, rm if needed
    docker container ls -a | grep idp_rdbms &>/dev/null
    if [  $? == '0' ]; then
        #get container ID
          export contid2=$(docker container ls -a | grep idp_rdbms | cut -f 1 -d ' ')
          docker kill ${contid2} &>/dev/null
          docker rm ${contid2} &>/dev/null
    fi
  fi
  ...
  docker rmi -f idp_rdbms &>/dev/null
